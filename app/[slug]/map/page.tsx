...
export default async function SubMapPage({ params }: { params: { slug: string } }) {
  ...
  for (const p of availableProjects) {
    ...
    points.push({
      ...
      subtitle: extractCity(p.address, tenant.service_area),
      status: 'available',
  ...
  if (areaCenter) {
    for (const p of unplacedProjects) {
      ...
      points.push({
        ...
        lng: spot.lng,
        title: p.customer_name,
        subtitle: owned ? [p.job_number ? `Job ${p.job_number}` : null, p.address].filter(Boolean).join(' · ') : extractCity(p.address, tenant.service_area),
