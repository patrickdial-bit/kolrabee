...
export async function joinAdminAction(
  ...
  // Require a valid pending admin invite — prevents unsolicited admin sign-ups.
  ...
  if (!invite) {
    return { error: 'No valid admin invitation found for this email. Please ask an admin to send you a new invite.' }
  }

  if (invite.expires_at && new Date(invite.expires_at) <= new Date()) {
    return { error: 'This admin invitation has expired. Please ask an admin to send you a new invite.' }
  }
